         ...
         x-data="{ 
            ...
            openAbstractModal(repo, badge) {
                this.abstractData = {
                    ...
                    file_path_bab6: repo.file_path_bab6 || '',
                    badge: badge || {}
                };
            ...
            async openPdfReader(repo, chapter = 'bab1') {
                ...
                if (chapter === 'bab6' && repo.file_path_bab6) {
                    this.pdfChapter = 'bab6';
                } else if (chapter === 'bab5' && repo.file_path_bab5) {
            ...
            async switchChapter(chapter) {
                ...
                if (chapter === 'bab6' && !this.pdfRepo.file_path_bab6) return;
                ...
                <div class="flex items-center gap-3 flex-wrap">
                    @if($search || $year || $advisor || ($topic && $topic !== 'all'))
                        <a href="{{ route('repositories.index') }}" 
                           class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800/80 rounded-xl text-xs font-bold transition-all shadow-2xs hover:scale-[1.02] active:scale-95 shrink-0"
                           title="Hapus pencarian dan kembalikan ke seluruh pustaka">
        ...
        <div x-show="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex justify-between items-center gap-2 mb-3">
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    @if($repo->file_path_bab6)
                                        <button type="button" 
                                                @click="openPdfReader({{ json_encode($repo) }}, 'bab6')" 
                                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider bg-cyan-50 dark:bg-cyan-950/60 text-cyan-700 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-800/80 hover:bg-cyan-100 dark:hover:bg-cyan-900/60 transition-all shadow-2xs hover:scale-105 active:scale-95 cursor-pointer" 
                                                title="Baca Naskah BAB 6 di Web (In-App Reader)">
                                            <svg class="w-3 h-3 text-cyan-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                                            <span>BAB 6 PDF</span>
                                        </button>
                                    @endif
                                </div>
            ...
            @empty
                <div class="col-span-full py-16 text-center bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 space-y-3">
                    ...
                    <div class="pt-2">
                        <a href="{{ route('repositories.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold rounded-xl transition-all shadow-sm shadow-orange-500/25 hover:scale-[1.02] active:scale-95">
                            <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                            <span>Reset Pencarian</span>
                        </a>
                    </div>
                                    ...
                                    <div class="space-y-1.5">
                                        ...
                                        <div class="flex items-center gap-2.5 flex-wrap">
                                            ...
                                            @if($repo->file_path_bab6)
                                                <button type="button" 
                                                        @click="openPdfReader({{ json_encode($repo) }}, 'bab6')" 
                                                        class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider bg-cyan-50 dark:bg-cyan-950/60 text-cyan-700 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-800/80 hover:bg-cyan-100 dark:hover:bg-cyan-900/60 transition-all shadow-2xs hover:scale-105 active:scale-95 cursor-pointer" 
                                                        title="Baca Naskah BAB 6 di Web (In-App Reader)">
                                                    <svg class="w-3 h-3 text-cyan-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                                                    <span>BAB 6 PDF</span>
                                                </button>
                            ...
                            <tr>
                                <td colspan="4" class="py-12 text-center text-slate-500 dark:text-slate-400">
                                    <div class="space-y-3 max-w-sm mx-auto">
                                        <p class="text-xs">Tidak ada arsip skripsi yang cocok dengan filter pencarian.</p>
                                        <a href="{{ route('repositories.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold rounded-xl transition-all shadow-sm shadow-orange-500/25 hover:scale-[1.02] active:scale-95">
                                            <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                                            <span>Reset Pencarian</span>
                                        </a>
                                    </div>
                    ...
                    <div class="inline-block align-bottom bg-white dark:bg-slate-900 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl w-full border border-slate-200 dark:border-slate-700 relative" 
                        ...
                        <div class="px-8 py-5 bg-slate-50/75 dark:bg-slate-900/60 border-t border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4">
                            <div class="flex items-center gap-3.5 sm:gap-4 flex-wrap">
                                <template x-if="abstractData.file_path">
                                    <div style="background-color: #e11d48;" 
                                        ...
                                        <button type="button" 
                                                ...
                                                class="inline-flex items-center gap-2.5 pl-4 pr-3.5 py-2.5 text-white text-xs font-bold hover:bg-white/10 active:bg-white/20 transition-colors cursor-pointer"
                                                title="Baca Naskah BAB 1 di Web (In-App Reader)">
                                            <svg class="w-4 h-4 text-white shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                                            <span>Baca BAB 1</span>
                                        </button>
                                        <span class="w-px h-4 bg-white/30 shrink-0" aria-hidden="true"></span>
                                        <a :href="'/repositories/' + abstractData.id + '/bab1'" 
                                           target="_blank" 
                                           class="inline-flex items-center justify-center pl-3 pr-4 py-2.5 text-white/90 hover:text-white hover:bg-white/10 active:bg-white/20 transition-colors cursor-pointer" 
                                ...
                                <template x-if="!abstractData.file_path && !abstractData.file_path_bab2 && !abstractData.file_path_bab3 && !abstractData.file_path_bab4 && !abstractData.file_path_bab5 && !abstractData.file_path_bab6">
                ...
                <header style="background-color: #0f172a !important; border-bottom: 1px solid #1e293b !important; color: #f8fafc !important; flex: 0 0 auto !important;" 
                    ...
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div style="background-color: #020617 !important; border: 1px solid #1e293b !important;" 
                            ...
                            <button type="button" 
                                    @click="switchChapter('bab6')" 
                                    :disabled="!pdfRepo.file_path_bab6"
                                    :style="pdfChapter === 'bab6' ? 'background-color: #0891b2 !important; color: #ffffff !important;' : 'color: #94a3b8 !important;'"
                                    class="px-2 sm:px-2.5 py-1 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
                                    title="BAB 6">
                                <span>BAB 6</span>
                            </button>
                ...
                <div style="position: relative !important; flex: 1 1 0% !important; min-height: 0 !important; width: 100% !important; overflow: hidden !important; display: flex !important; flex-direction: column !important;">
                    ...
                    <div x-show="pdfIsLoading" 
                        ...
                        <p class="text-xs text-slate-300 mt-1.5 font-medium text-center max-w-sm mx-auto" 
                           x-text="({'bab1':'Menyiapkan BAB 1 (Pendahuluan)','bab2':'Menyiapkan BAB 2 (Tinjauan Pustaka)','bab3':'Menyiapkan BAB 3 (Metodologi Penelitian)','bab4':'Menyiapkan BAB 4 (Hasil dan Pembahasan)','bab5':'Menyiapkan BAB 5 (Implementasi / Kesimpulan)','bab6':'Menyiapkan BAB 6 (Kesimpulan dan Saran)'})[pdfChapter] || 'Menyiapkan Naskah PDF'"></p>
                    </div>
