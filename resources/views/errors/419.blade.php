...
@section('code', '419')
@section('status_badge', 'Page Expired')

@section('message')
    Sesi keamanan formulir telah kedaluwarsa karena tidak ada aktivitas dalam waktu lama. Silakan muat ulang halaman untuk memperbarui token keamanan.
@endsection

@section('actions')
<button onclick="window.location.reload()" 
        type="button"
        class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 bg-orange-600 hover:bg-orange-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-md shadow-orange-600/20 hover:shadow-orange-600/30 active:scale-95 transition-all cursor-pointer whitespace-nowrap">
...
<div class="pt-8 border-t border-slate-200/80 dark:border-slate-800 text-xs font-semibold text-slate-500 dark:text-slate-400">
    <span>💡 Tip: Muat ulang halaman sebelum mengisi formulir yang telah lama terbuka.</span>
</div>
@endsection
