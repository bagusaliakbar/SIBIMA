    ...
    <div class="w-full space-y-6" x-data="{ allOpen: true }">
        ...
        @if(Auth::user()->role === 'dosen')
            ...
            <div class="flex items-center gap-1 border-b border-slate-200 dark:border-slate-700 overflow-x-auto pb-px custom-scrollbar">
                ...
                <!-- "Jadwal Saya" Tab -->
                <a href="{{ route('thesis-defense-schedules.index', array_merge(request()->except(['page', 'my_schedules']), ['my_schedules' => $mySchedules ? '0' : '1', 'filter_date' => 'all'])) }}"
            ...
            <div class="p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-xs flex flex-col lg:flex-row items-center justify-between gap-3">
                <form action="{{ route('thesis-defense-schedules.index') }}" method="GET" class="flex flex-wrap items-center gap-2.5 w-full lg:w-auto flex-1">
                    ...
                    <select name="wave_id" onchange="this.form.submit()"
                            class="py-2 pl-3 pr-8 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all min-w-[180px]">
                    ...
                    <button type="submit" class="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-xs flex items-center gap-1.5 shrink-0">
            ...
            <div class="space-y-4">
                @forelse($schedules as $schedule)
                    ...
                    <div x-data="{ open: true }" x-init="$watch('allOpen', value => open = value)"
                        ...
                        <!-- Header Banner -->
                        <div class="p-5 border-b border-slate-100 dark:border-slate-700/60 bg-gradient-to-r {{ count($myRoles) > 0 ? 'from-amber-50/40 via-white to-white dark:from-amber-950/20 dark:via-slate-800 dark:to-slate-800' : 'from-slate-50/80 via-white to-white dark:from-slate-800/80 dark:via-slate-800 dark:to-slate-800' }}">
                            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                                <div class="space-y-1.5 flex-1">
                                    <!-- Badges Row -->
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-orange-50 dark:bg-orange-500/10 text-orange-700 dark:text-orange-300 rounded-lg text-xs font-bold border border-orange-200 dark:border-orange-500/20">
                                    ...
                                    <!-- Session Title -->
                                    <h3 class="text-base font-black tracking-tight text-slate-800 dark:text-slate-100 uppercase">
                                        {{ $schedule->title }}
                                    </h3>
                                </div>
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    <a href="{{ route('thesis-defense-schedules.export-pdf', $schedule) }}" 
                                       class="inline-flex items-center px-3 py-1.5 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-600 rounded-xl text-xs font-bold shadow-2xs transition-all"
                                       title="Cetak PDF">
                                        <svg class="w-3.5 h-3.5 mr-1.5 text-rose-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 01-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                        Cetak PDF
                                    </a>

                                    <button type="button" @click="open = !open" 
                                            class="inline-flex items-center gap-1 px-3 py-1.5 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-bold transition-all"
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-3 mt-3 border-t border-slate-100 dark:border-slate-700/60 text-xs">
                                <div class="px-3 py-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700/60">
                                    <span class="block text-[9px] uppercase tracking-wider text-slate-400 font-bold">Ketua Sidang</span>
                                    <span class="font-bold text-slate-800 dark:text-slate-200 truncate block text-xs">{{ $schedule->chairman?->name ?? '-' }}</span>
                                </div>
                                <div class="px-3 py-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700/60">
                                    <span class="block text-[9px] uppercase tracking-wider text-slate-400 font-bold">Moderator</span>
                                    <span class="font-bold text-slate-800 dark:text-slate-200 truncate block text-xs">{{ $schedule->moderator?->name ?? '-' }}</span>
                                </div>
                                <div class="px-3 py-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700/60">
                                    <span class="block text-[9px] uppercase tracking-wider text-slate-400 font-bold">Ruangan</span>
                                    <span class="font-bold text-slate-800 dark:text-slate-200 truncate block text-xs">{{ $schedule->location ?: 'BELUM DIATUR' }}</span>
                                </div>
                                <div class="px-3 py-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700/60">
                        ...
                        <!-- Presentation Table -->
                        <div x-show="open" x-transition.opacity.duration.150ms class="p-5">
                            <div class="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xs">
                                ...
                                <td class="py-4 px-6 text-right">
                                    <div class="flex justify-end items-center gap-1">
                                        <a href="{{ route('thesis-defense-schedules.export-pdf', $schedule) }}" class="p-2 text-slate-400 hover:text-rose-600 transition-all group/pdf" title="Export PDF">
                                            <svg class="w-5 h-5 group-hover/pdf:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                        </a>
