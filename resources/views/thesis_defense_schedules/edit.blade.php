<x-app-layout>
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 rounded-lg overflow-hidden mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="date" name="date" type="date" @change="recheckAll()" class="mt-1 block w-full text-sm" value="{{ \Carbon\Carbon::parse($thesisDefenseSchedule->date)->format('Y-m-d') }}" required />
                    ...
                    <div>
                        <x-google-meet-input name="meeting_link" id="meeting_link" :value="old('meeting_link', $thesisDefenseSchedule->meeting_link)" label="Link Meeting (Google Meet / Zoom)" />
    ...
    <script>
        function scheduleForm() {
            return {
                chairman_id: '{{ $thesisDefenseSchedule->chairman_id }}',
                moderator_id: '{{ $thesisDefenseSchedule->moderator_id }}',
                    ...
                    try {
                        const response = await fetch('{{ route("check-dosen-availability") }}', {
                            ...
                            body: JSON.stringify({
                                ...
                                current_schedule_id: '{{ $thesisDefenseSchedule->id }}'
                            })
                        });
