<x-app-layout>
    ...
    <div class="w-full mx-auto" x-data="scheduleForm()">
        <form action="{{ route('thesis-defense-schedules.store') }}" method="POST">
            @csrf
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 rounded-lg overflow-hidden mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="date" name="date" type="date" class="mt-1 block w-full text-sm" required />
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 rounded-lg overflow-hidden">
                <div class="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-900/30 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1 uppercase font-medium text-xs">Tambahkan urutan waktu, kegiatan, atau mahasiswa yang akan sidang</p>
                    </div>
                ...
                <div class="p-6">
                    <div class="space-y-4">
                            ...
                            <div class="p-4 border rounded-lg dark:border-slate-700 relative bg-slate-50/20 dark:bg-slate-900/10" :class="row.type === 'student' ? 'border-orange-100 dark:border-orange-900/30' : 'border-slate-100 dark:border-slate-700'">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <input type="time" :name="`details[${index}][start_time]`" class="w-full border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 rounded-md text-xs focus:ring-orange-500 focus:border-orange-500" required>
                                    </div>
                                    <div class="md:col-span-2">
                                        <x-input-label value="Waktu Selesai" class="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1" />
                                        <input type="time" :name="`details[${index}][end_time]`" class="w-full border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 rounded-md text-xs focus:ring-orange-500 focus:border-orange-500" required>
                                    </div>
                                    ...
                                    <div class="md:col-span-8" x-show="row.type === 'activity'">
                                        ...
                                        <input type="text" :name="`details[${index}][activity_name]`" class="w-full border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 rounded-md text-xs focus:ring-orange-500 focus:border-orange-500" placeholder="Contoh: Persiapan dan Pembukaan Sesi Sidang">
                                    </div>
                                    ...
                                    <div class="md:col-span-4" x-show="row.type === 'student'">
                                        ...
                                        <select :name="`details[${index}][thesis_id]`" class="w-full border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 rounded-md text-xs focus:ring-orange-500 focus:border-orange-500" @change="row.thesis_id = $event.target.value">
                        ...
                        <div x-show="rows.length === 0" class="py-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl">
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Klik tombol tambah di atas untuk menyusun jadwal</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function scheduleForm() {
            return {
                rows: [
                    { type: 'activity', start_time: '', end_time: '', activity_name: '', thesis_id: '', examiner1_id: '', examiner2_id: '' }
                ],
                theses: @json($theses),
                addRow(type) {
                    this.rows.push({
