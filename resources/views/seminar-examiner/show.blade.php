        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800/50 dark:backdrop-blur-xl rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700/50 flex flex-col h-[600px]">
                                    ...
                                    <div class="p-4 rounded-2xl shadow-sm text-sm font-medium leading-relaxed {{ $isMe ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-tl-none border border-slate-100 dark:border-slate-700' }}">
                                        ...
                                        @if($msg->file_path)
                                            <div class="mt-3 pt-3 border-t {{ $isMe ? 'border-white/10' : 'border-slate-50 dark:border-slate-700' }}">
                                                <a href="{{ Storage::url($msg->file_path) }}" target="_blank" class="inline-flex items-center text-[10px] font-black uppercase tracking-widest {{ $isMe ? 'text-indigo-100 hover:text-white' : 'text-indigo-600 hover:text-indigo-700' }}">
                ...
                <div class="p-6 border-t border-slate-50 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-b-2xl">
                    ...
                    @else
                        <form action="{{ route('seminar-examiner.store-revision', $detail->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            @csrf
                            <div class="relative">
                            ...
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div x-data="{ fileName: '' }" class="relative">
                                    ...
                                    <label for="revision_file" class="inline-flex items-center text-[10px] font-black text-slate-500 hover:text-indigo-600 cursor-pointer uppercase tracking-widest transition-colors">
                                ...
                                <div class="flex items-center gap-3">
                                    @if($myRevision && $myRevision->status === 'resubmitted')
                                    ...
                                    @endif
                                    <button type="submit" class="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all">
                                        Kirim Feedback
                                    </button>
                                </div>
                            </div>
