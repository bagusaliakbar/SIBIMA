        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <h2 class="font-black text-2xl text-slate-800 dark:text-slate-100 leading-tight tracking-tight flex items-center">
                    Diskusi & Tindak Lanjut Revisi Sidang
                </h2>
                <p class="text-[11px] font-medium text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest flex items-center">
                    Pantau riwayat revisi hasil sidang dan tindak lanjuti masukan penguji
                </p>
            </div>
            <a href="{{ route('student-defense-revisions.index') }}" class="inline-flex items-center px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-xs text-slate-600 dark:text-slate-300 uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-sm">
                                ...
                                <div class="bg-rose-50/50 dark:bg-rose-500/5 p-4 rounded-xl border border-rose-100 dark:border-rose-500/20 shadow-sm">
                                    ...
                                    <div class="mt-3 flex items-center gap-3">
                                        @if($firstExaminerMessage->file_path)
                                            <a href="{{ Storage::url($firstExaminerMessage->file_path) }}" target="_blank" class="inline-flex items-center text-[9px] font-black text-rose-600 uppercase tracking-widest hover:underline">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800/50 dark:backdrop-blur-xl rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700/50 flex flex-col h-[600px]">
                                ...
                                <div class="p-4 rounded-2xl shadow-sm text-sm font-medium leading-relaxed {{ $isMe ? 'bg-rose-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-tl-none border border-slate-100 dark:border-slate-700' }}">
                                    ...
                                    @if($msg->file_path)
                                        <div class="mt-3 pt-3 border-t {{ $isMe ? 'border-white/10' : 'border-slate-50 dark:border-slate-700' }}">
                                            <a href="{{ Storage::url($msg->file_path) }}" target="_blank" class="inline-flex items-center text-[10px] font-black uppercase tracking-widest {{ $isMe ? 'text-rose-100 hover:text-white' : 'text-rose-600 hover:text-rose-700' }}">
                ...
                <div class="p-6 border-t border-slate-50 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-b-2xl">
                    ...
                    @else
                        <form action="{{ route('student-defense-revisions.store-reply', $revision->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            @csrf
                            <div class="relative">
                                <textarea name="student_notes" rows="3" class="block w-full rounded-2xl border-slate-200 dark:border-slate-700 dark:bg-slate-900/50 dark:text-slate-200 focus:border-rose-500 focus:ring-rose-500 text-sm font-medium transition-all pr-12" placeholder="Tuliskan penjelasan perbaikan hasil sidang..." required></textarea>
                            </div>
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div x-data="{ fileName: '' }" class="relative">
                                    ...
                                    <label for="student_file" class="inline-flex items-center text-[10px] font-black text-slate-500 hover:text-rose-600 cursor-pointer uppercase tracking-widest transition-colors">
