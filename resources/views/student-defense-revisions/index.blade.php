                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="bg-slate-50/50 dark:bg-slate-900/50 text-slate-400 text-[10px] font-bold uppercase tracking-widest border-b border-slate-50 dark:border-slate-700">
                            ...
                            <th class="px-6 py-4">Tgl Sidang</th>
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group">
                            <td class="px-6 py-5">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-slate-800 dark:text-slate-100 text-sm group-hover:text-rose-600 transition-colors">{{ $revision->examiner->name }}</h4>
                                        @if($revision->detail->thesis->pembimbing1_id === $revision->examiner_id)
                                            <p class="text-[10px] text-rose-500 mt-1 font-bold uppercase tracking-tighter">Pembimbing Utama</p>
                                        @elseif($revision->detail->thesis->pembimbing2_id === $revision->examiner_id)
                                            <p class="text-[10px] text-rose-500 mt-1 font-bold uppercase tracking-tighter">Pembimbing Pendamping</p>
                                        @else
                            ...
                            <td class="px-6 py-5">
                                @if($revision->status === 'approved')
                                    ...
                                    <p class="text-[8px] text-slate-400 mt-1 font-bold">Telah Disetujui</p>
                                @elseif($revision->resubmitted_at)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest bg-blue-100 text-blue-700 border border-blue-200">
                                        Terkirim
                                    </span>
                            ...
                            <td class="px-6 py-5 text-right">
                                <a href="{{ route('student-defense-revisions.show', $revision->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 hover:bg-rose-600 hover:text-white dark:hover:bg-rose-600 transition-all shadow-sm">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-slate-400 font-bold text-sm uppercase tracking-widest">Belum Ada Revisi Sidang</h3>
                                    <p class="text-[11px] text-slate-400 mt-1 italic">Dosen penguji belum mengunggah catatan revisi untuk sidang Anda.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
