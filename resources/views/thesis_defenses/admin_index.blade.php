<x-app-layout>
    ...
    <div class="w-full space-y-6" 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200/80 dark:border-slate-700/80 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div x-data="{ open: false }">
                    ...
                    <div x-show="open" class="fixed inset-0 z-[150] overflow-y-auto" x-cloak x-transition>
        ...
        {{-- GLOBAL VIEW FILES MODAL (OUTSIDE TABLE, PERFECTLY CENTERED) --}}
        <div x-show="showFiles" class="fixed inset-0 z-[150] overflow-y-auto" x-cloak x-transition>
            <div class="flex items-center justify-center min-h-screen px-4 py-8 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl w-full border border-slate-100 dark:border-slate-700 relative z-10">
                    <template x-if="selectedApp">
                        <div>
                            <div class="px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                                <div>
                            ...
                            <div class="p-6 sm:p-8 max-h-[70vh] overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-3.5">
                                @foreach($fileLabels as $field => $label)
                                    <div class="p-3.5 bg-slate-50 dark:bg-slate-900/70 border rounded-2xl flex items-center justify-between gap-3 shadow-2xs"
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <template x-if="selectedApp.file_reviews && selectedApp.file_reviews['{{ $field }}'] && selectedApp.file_reviews['{{ $field }}'].status === 'rejected'">
                                                <span class="text-[10px] font-black text-rose-600 dark:text-rose-400 mt-0.5 block" x-text="'Ditolak: ' + (selectedApp.file_reviews['{{ $field }}'].note || 'Perlu revisi')"></span>
                                            </template>
                                        </div>
                                        <a :href="selectedApp.files['{{ $field }}']" target="_blank" 
                                           class="inline-flex items-center gap-1 px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-indigo-400 dark:hover:border-indigo-500 rounded-lg text-[10px] font-bold text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors shrink-0 shadow-2xs">
                                            <span>Buka File</span>
                            ...
                            <div class="px-6 sm:px-8 py-4 bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex justify-end">
        ...
        {{-- GLOBAL WIDE 2-COLUMN VALIDATION MODAL (OUTSIDE TABLE, PERFECTLY CENTERED) --}}
        <div x-show="openValidation" class="fixed inset-0 z-[150] overflow-y-auto" x-cloak x-transition>
            <div class="flex items-center justify-center min-h-screen px-4 py-8 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-5xl w-full border border-slate-100 dark:border-slate-700 relative z-10">
                    <template x-if="selectedApp">
                        <div>
                            {{-- Modal Header --}}
                            <div class="px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/70 dark:bg-slate-900/60">
                                <div>
                                    <div class="flex items-center gap-2">
                                        <h3 class="text-base font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight">Validasi Berkas Sidang Skripsi</h3>
                                ...
                                <div class="px-6 sm:px-8 py-6 space-y-6 max-h-[68vh] overflow-y-auto">
                                    ...
                                    <div>
                                        <div class="flex items-center justify-between mb-3">
                                            ...
                                            <span class="text-[10px] text-slate-400 font-medium">Klik nama berkas untuk melihat isi file</span>
                                        </div>
                                    ...
                                    <div class="pt-5 border-t border-slate-100 dark:border-slate-700 grid grid-cols-1 md:grid-cols-2 gap-5 bg-slate-50/50 dark:bg-slate-900/30 p-4 rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
                                        <div>
                                            ...
                                            <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-1.5">
                                                Pilih "SETUJUI" jika seluruh 20 berkas telah lengkap dan sesuai syarat.
                                            </p>
                                        </div>
