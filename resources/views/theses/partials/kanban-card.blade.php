...
<div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xs border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all cursor-pointer group" 
    ...
    <div class="flex items-center justify-between gap-3" style="width: 100%; min-width: 0; margin-bottom: 12px;">
        <div class="flex items-center gap-2.5 min-w-0" style="flex: 1; overflow: hidden;">
            <div class="w-8 h-8 rounded-full overflow-hidden bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-100 dark:border-indigo-900/50 flex items-center justify-center shrink-0 font-black text-indigo-600 dark:text-indigo-400 text-xs" style="width: 32px; height: 32px; flex-shrink: 0;">
            ...
            <div style="flex: 1; min-width: 0; overflow: hidden;">
                ...
                <div class="flex items-center gap-1.5" style="margin-top: 2px; min-width: 0; overflow: hidden;">
                    <span style="font-size: 10px; font-family: monospace; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" class="text-slate-400">
                        {{ $thesis->student->identifier ?? 'NPM -' }}
                    </span>
                    @if($thesis->student && $thesis->student->entry_year)
                        <span style="font-size: 9px; font-weight: 700; white-space: nowrap;" class="text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.2 rounded shrink-0">
    ...
    <div style="width: 100%; min-width: 0; overflow: hidden; margin-top: 10px; margin-bottom: 12px;">
        <h5 style="font-size: 11px; font-weight: 600; line-height: 1.5; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; word-break: break-word; margin: 0;" class="text-slate-700 dark:text-slate-300 uppercase" title="{{ $thesis->final_title ?? $thesis->title }}">
        ...
        @if($simScore > 0)
            <div style="margin-top: 8px;" class="flex items-center">
                @if($simScore >= 60)
                    <span style="font-size: 9px; font-weight: 800; padding: 3px 8px;" class="inline-flex items-center gap-1.5 rounded-md uppercase tracking-wider bg-rose-50 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400 border border-rose-200/60 dark:border-rose-900/40">
                        <span class="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse shrink-0"></span>
                        {{ $simScore }}% SANGAT MIRIP
                    </span>
                @elseif($simScore >= 40)
                    <span style="font-size: 9px; font-weight: 800; padding: 3px 8px;" class="inline-flex items-center gap-1.5 rounded-md uppercase tracking-wider bg-amber-50 text-amber-600 dark:bg-amber-950/40 dark:text-amber-400 border border-amber-200/60 dark:border-amber-900/40">
                        <span class="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0"></span>
                        {{ $simScore }}% MIRIP
                    </span>
                @else
                    <span style="font-size: 9px; font-weight: 800; padding: 3px 8px;" class="inline-flex items-center gap-1.5 rounded-md uppercase tracking-wider bg-emerald-50 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-900/40">
    ...
    <!-- Supervisors Section -->
    <div style="width: 100%; min-width: 0; padding-top: 12px; border-top: 1px solid #f1f5f9;" class="dark:border-slate-800">
        <!-- Pembimbing 1 -->
        <div class="flex items-center gap-2" style="width: 100%; min-width: 0; overflow: hidden; margin-bottom: 6px;">
            <span style="background-color: #e0e7ff; color: #3730a3; font-size: 8px; font-weight: 900; padding: 2px 6px; border-radius: 4px;" class="uppercase shrink-0">P1</span>
        ...
        <div class="flex items-center gap-2" style="width: 100%; min-width: 0; overflow: hidden;">
            <span style="background-color: #f3e8ff; color: #6b21a8; font-size: 8px; font-weight: 900; padding: 2px 6px; border-radius: 4px;" class="uppercase shrink-0">P2</span>
