...
<div class="bg-white dark:bg-slate-900 p-3.5 rounded-2xl shadow-2xs border border-slate-200/80 dark:border-slate-800 hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-200 cursor-pointer border-l-4 {{ $accentBorder }} group relative" 
        ...
        <div class="flex items-center gap-2 min-w-0">
            ...
            <div class="min-w-0">
                ...
                <div class="flex items-center gap-1.5 mt-0.5">
                    <span class="text-[9px] text-slate-400 font-mono font-medium">{{ $thesis->student->identifier ?? 'NPM -' }}</span>
                    @if($thesis->student && $thesis->student->entry_year)
                        <span class="text-[8px] font-medium text-slate-400 bg-slate-100 dark:bg-slate-800 px-1 py-0.2 rounded">
    ...
    <!-- Title Section -->
    <div class="my-2">
        <h5 class="text-[11px] font-normal text-slate-600 dark:text-slate-300 leading-snug line-clamp-2" title="{{ $thesis->final_title ?? $thesis->title }}">
    ...
    <div class="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1">
        ...
        <div class="flex items-center gap-1.5 text-[10px] min-w-0">
            ...
            @if($thesis->pembimbing1)
                <span class="font-medium text-slate-600 dark:text-slate-300 text-[10px] truncate">{{ $thesis->pembimbing1->name }}</span>
            @else
                <span class="font-medium text-amber-600 dark:text-amber-400 text-[9px] italic">Belum Ditentukan</span>
            @endif
        </div>
        ...
        <div class="flex items-center gap-1.5 text-[10px] min-w-0">
            ...
            @if($thesis->pembimbing2)
                <span class="font-medium text-slate-600 dark:text-slate-300 text-[10px] truncate">{{ $thesis->pembimbing2->name }}</span>
            @else
                <span class="font-medium text-amber-600 dark:text-amber-400 text-[9px] italic">Belum Ditentukan</span>
            @endif
        </div>
