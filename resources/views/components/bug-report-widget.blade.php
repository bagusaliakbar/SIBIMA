<div x-data="bugReportWidget()" 
    ...
    <div x-cloak
         class="fixed bottom-6 right-6 z-[99990] flex flex-col items-center select-none"
        ...
        <div x-show="!isCollapsed"
            ...
            <button @click="isCollapsed = true"
                    type="button"
                    class="w-10 h-10 rounded-full bg-slate-100/90 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 shadow-2xs group relative"
                ...
                <!-- Tooltip Label -->
                <span class="absolute right-13 bg-slate-900 text-white text-[11px] font-semibold px-2.5 py-1 rounded-md opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap shadow-md">
                    Perkecil
                </span>
            </button>
            ...
            <button @click="showGuideModal = true"
                    type="button"
                    class="w-10 h-10 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 shadow-md shadow-orange-500/25 group relative"
            ...
            <button @click="hideWidget()"
                    type="button"
                    class="w-10 h-10 rounded-full bg-rose-500 hover:bg-rose-600 text-white flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 shadow-md shadow-rose-500/25 group relative"
            ...
            <button @click="openMyReportsModal()"
                    type="button"
                    class="w-10 h-10 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 shadow-md shadow-emerald-500/25 group relative"
            ...
            <button @click="openReportModal()"
                    type="button"
                    class="w-10 h-10 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 shadow-lg shadow-orange-500/30 group relative"
    ...
    <div x-cloak
        ...
        <button @click="unhideWidget()"
                type="button"
                class="flex items-center gap-2 px-3.5 py-2.5 bg-gradient-to-r from-orange-500 to-amber-500 text-white text-xs font-bold rounded-full shadow-xl shadow-orange-500/25 hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer">
    ...
    <!-- ========================================================================= -->
    <!-- MODAL 1: FORMULIR PELAPORAN BUG (TELEPORTED TO BODY)                       -->
    <!-- ========================================================================= -->
        ...
        <div x-show="showReportModal"
            ...
            <div class="min-h-screen px-4 py-8 flex items-center justify-center">
                <div x-show="showReportModal"
                     ...
                     class="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden relative z-10">
                    ...
                    <form @submit.prevent="submitReport" class="p-6 sm:p-8 space-y-5 max-h-[75vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <button type="button" 
                                        ...
                                        class="text-[11px] font-semibold text-orange-600 dark:text-orange-400 hover:underline">
                                    Ambil URL Sekarang
                                </button>
                            </div>
                            <input type="text"
                                ...
                                <template x-if="screenshotPreview">
                                    <div class="relative w-full flex flex-col items-center">
                                        ...
                                        <button type="button" 
                                                ...
                                                class="mt-2 text-xs text-rose-600 dark:text-rose-400 font-bold hover:underline flex items-center gap-1">
                        ...
                        <!-- Error Alert (If Any) -->
                        <div x-show="errorMessage" x-cloak class="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900 text-xs font-semibold text-rose-600 dark:text-rose-400" x-text="errorMessage"></div>
    ...
    <!-- ========================================================================= -->
    <!-- MODAL 2: STATUS & RIWAYAT LAPORAN SAYA (TELEPORTED TO BODY)               -->
    <!-- ========================================================================= -->
    <template x-teleport="body">
        <div x-show="showHistoryModal"
            ...
            <div class="min-h-screen px-4 py-8 flex items-center justify-center">
                <div x-show="showHistoryModal"
                     ...
                     x-transition:leave-start="opacity-100 scale-100 translate-y-0"
                     x-transition:leave-end="opacity-0 scale-95 translate-y-4"
                     class="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden relative z-10 flex flex-col max-h-[85vh]">
                    ...
                    <div class="px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-emerald-50/60 via-teal-50/40 to-transparent dark:from-emerald-950/20 dark:via-transparent shrink-0">
                        ...
                        <div class="flex items-center gap-2">
                            <button @click="fetchMyReports()"
                                    type="button"
                                    title="Segarkan Data"
                                    class="w-8 h-8 rounded-lg text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center transition-colors">
                            ...
                            <button @click="showHistoryModal = false"
                                    type="button"
                                    class="w-8 h-8 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                            </button>
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            ...
                                            <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider shrink-0"
                                                  :class="{
                                                      ...
                                                      'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400 border border-slate-200 dark:border-slate-700': item.status === 'rejected',
                                                  }"
                    ...
                    <div class="p-4 px-6 sm:px-8 bg-slate-50 dark:bg-slate-800/60 border-t border-slate-100 dark:border-slate-800 flex justify-end shrink-0">
                        <button type="button" 
                                @click="showHistoryModal = false" 
                                class="px-5 py-2 bg-slate-800 dark:bg-white text-white dark:text-slate-800 text-xs font-bold rounded-xl uppercase tracking-wider hover:bg-slate-900 transition-all shadow-xs">
                            Tutup
                        </button>
                    </div>
                </div>
    ...
    <!-- ========================================================================= -->
    <!-- MODAL 3: PANDUAN PELAPORAN BUG (TELEPORTED TO BODY)                       -->
    <!-- ========================================================================= -->
    <template x-teleport="body">
        <div x-show="showGuideModal"
            ...
            <div class="min-h-screen px-4 py-8 flex items-center justify-center">
                <div x-show="showGuideModal"
                     ...
                     x-transition:leave-start="opacity-100 scale-100 translate-y-0"
                     x-transition:leave-end="opacity-0 scale-95 translate-y-4"
                     class="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden relative z-10">
                    ...
                    <div class="px-6 pt-6 pb-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-amber-50/60 to-transparent dark:from-amber-950/20">
                        ...
                        <button @click="showGuideModal = false"
                                type="button"
                                class="w-8 h-8 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white flex items-center justify-center">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                            </svg>
                        </button>
                    ...
                    <div class="p-4 px-6 bg-slate-50 dark:bg-slate-800/60 border-t border-slate-100 dark:border-slate-800 flex justify-end">
                        <button type="button"
                                @click="showGuideModal = false"
                                class="px-5 py-2 bg-orange-600 text-white text-xs font-bold rounded-xl uppercase tracking-wider hover:bg-orange-700 transition-colors shadow-xs">
                            Mengerti
                        </button>
                    </div>
                </div>
...
function bugReportWidget() {
    return {
        ...
        init() {
            // Restore hidden/collapse state from sessionStorage if preferred
