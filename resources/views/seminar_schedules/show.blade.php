                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                    <div class="space-y-3">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm font-bold text-slate-800 dark:text-slate-200">: {{ \Carbon\Carbon::parse($seminarSchedule->date)->locale('id')->translatedFormat('l, d F Y') }}</span>
                        </div>
                        <div class="flex items-center">
                            <span class="w-32 text-[10px] font-black text-slate-400 uppercase tracking-widest">Ketua Sidang</span>
                            <span class="text-sm font-bold text-slate-800 dark:text-slate-200">: {{ $seminarSchedule->chairman?->name ?? '-' }}</span>
                        </div>
                        <div class="flex items-center">
                            <span class="w-32 text-[10px] font-black text-slate-400 uppercase tracking-widest">Moderator</span>
                            <span class="text-sm font-bold text-slate-800 dark:text-slate-200">: {{ $seminarSchedule->moderator?->name ?? '-' }}</span>
                        </div>
                    </div>
                    <div class="flex flex-col md:items-end justify-start space-y-3">
                        <div class="inline-flex items-center px-4 py-2 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700">
                            ...
                            <span class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ $seminarSchedule->location ?: '-' }}</span>
                        </div>
                        @if($seminarSchedule->meeting_link)
                        <a href="{{ $seminarSchedule->meeting_link }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg border border-orange-700 transition-colors shadow-sm">
                            ...
                            <span class="text-xs font-black uppercase tracking-widest">Join Google Meet</span>
                        </a>
                        @endif
                    </div>
                </div>
