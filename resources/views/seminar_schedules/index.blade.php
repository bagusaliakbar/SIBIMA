    ...
    <div class="w-full mx-auto">
        ...
        <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700 rounded-lg">
            <div class="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-900/30">
                <h3 class="text-base font-bold text-slate-800 dark:text-slate-100 uppercase tracking-widest">Daftar Jadwal Seminar</h3>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 uppercase font-medium">Manajemen seluruh jadwal seminar yang telah diterbitkan</p>
            </div>
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group">
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex flex-col">
                                        <span class="text-sm font-bold text-slate-700 dark:text-slate-200">{{ \Carbon\Carbon::parse($schedule->date)->translatedFormat('d F Y') }}</span>
                                        <span class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-medium">{{ \Carbon\Carbon::parse($schedule->date)->translatedFormat('l') }}</span>
                                    </div>
                                </td>
                                <td class="py-4 px-6">
                                    <div class="space-y-1">
                                ...
                                <td class="py-4 px-6 text-right">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('seminar-schedules.export-pdf', $schedule) }}" class="p-2 text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors" title="Download PDF">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                        </a>
                                        <a href="{{ route('seminar-schedules.show', $schedule) }}" class="p-2 text-slate-400 hover:text-orange-600 dark:hover:text-orange-400 transition-colors" title="Detail">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        </a>
                                        <a href="{{ route('seminar-schedules.edit', $schedule) }}" class="p-2 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors" title="Edit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form action="{{ route('seminar-schedules.destroy', $schedule) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus jadwal ini?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-20 text-center">
                                    ...
                                    <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-1 uppercase font-black">Klik tombol di atas untuk membuat jadwal seminar pertama</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
