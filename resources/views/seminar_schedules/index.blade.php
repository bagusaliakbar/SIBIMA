    ...
    <div class="w-full space-y-6" x-data="{ allOpen: true }">
        ...
        @if(Auth::user()->role === 'dosen')
            ...
            <div class="p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-xs flex flex-col lg:flex-row items-center justify-between gap-3">
                <form action="{{ route('seminar-schedules.index') }}" method="GET" class="flex flex-wrap items-center gap-2.5 w-full lg:w-auto flex-1">
                    ...
                    @if(request()->hasAny(['search', 'wave_id', 'date', 'date_from', 'date_to', 'my_schedules']) || ($filterDate && $filterDate !== 'upcoming'))
                        <a href="{{ route('seminar-schedules.index') }}" class="p-2 text-slate-400 hover:text-rose-600 rounded-xl transition-all" title="Reset Filter">
