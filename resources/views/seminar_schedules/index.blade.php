    ...
    <div class="w-full space-y-6" x-data="{ allOpen: true }">
        ...
        @if(Auth::user()->role === 'dosen')
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm p-6 space-y-5">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-700/60 pb-5">
                    <div>
                        <h2 class="text-lg font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight flex items-center gap-2">
                            ...
                            Agenda & Jadwal Seminar Proposal
                        </h2>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">
                            Menampilkan <span class="font-bold text-slate-700 dark:text-slate-200">{{ $schedules->total() }}</span> agenda pelaksanaan seminar
                        </p>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <!-- "Jadwal Saya" Toggle -->
                        <a href="{{ route('seminar-schedules.index', array_merge(request()->except(['page', 'my_schedules']), ['my_schedules' => $mySchedules ? '0' : '1'])) }}"
                ...
                <!-- Search & Filters Row -->
                <form action="{{ route('seminar-schedules.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-end">
                    ...
                    <div class="lg:col-span-1 flex items-center gap-1.5">
                        ...
                        @if(request()->hasAny(['search', 'wave_id', 'date', 'date_from', 'date_to', 'my_schedules']) || ($filterDate && $filterDate !== 'upcoming'))
                            <a href="{{ route('seminar-schedules.index') }}" class="p-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 rounded-xl transition-all" title="Reset Filter">
                            ...
                            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                                ...
                                <div class="flex items-center gap-3 shrink-0" @click.stop>
                                    <a href="{{ route('seminar-schedules.export-pdf', $schedule) }}" 
                                       class="inline-flex items-center px-3.5 py-1.5 bg-white/10 hover:bg-white/20 text-white border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
