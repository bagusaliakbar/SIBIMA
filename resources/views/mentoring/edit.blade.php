<x-app-layout>
    ...
    <div class="w-full space-y-6">
        <div class="bg-white dark:bg-slate-800 p-5 md:p-7 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-700 transition-colors space-y-6">
            ...
            @if(isset($relatedSessions) && $relatedSessions->count() > 0)
                <div class="p-4 sm:p-5 bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-200/80 dark:border-indigo-800/80 rounded-2xl space-y-3.5" x-data="{ expanded: false }">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h4 class="text-xs font-bold text-indigo-950 dark:text-indigo-100">
                                    Sesi Bimbingan Bersama / Kelompok ({{ $relatedSessions->count() + 1 }} Mahasiswa Terdaftar)
                                </h4>
                                <p class="text-[11px] text-indigo-700/90 dark:text-indigo-300/90">Sesi bimbingan massal pada tanggal dan jam yang sama.</p>
                            </div>
                        </div>
                        <span class="px-2.5 py-1 bg-indigo-200/80 dark:bg-indigo-900/80 text-indigo-800 dark:text-indigo-200 rounded-lg text-[10px] font-black uppercase tracking-wider shrink-0">
                            Bimbingan Bersama
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 pt-1">
                        <div class="flex items-center gap-2 p-2 bg-white dark:bg-slate-900 border-2 border-orange-400 dark:border-orange-500 rounded-xl shadow-2xs">
                        ...
                        @foreach($relatedSessions as $idx => $rel)
                            <div x-show="expanded || {{ $idx }} < 7" class="flex items-center gap-2 p-2 bg-white dark:bg-slate-900 border border-indigo-100 dark:border-indigo-800/60 rounded-xl shadow-2xs">
                    ...
                    @if($relatedSessions->count() > 7)
                        <div class="text-center pt-1">
                            <button type="button" 
                                    ...
                                    class="inline-flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/40 rounded-lg text-xs font-bold transition-all shadow-2xs cursor-pointer">
            ...
            <form action="{{ route('mentoring-sessions.update', $mentoringSession) }}" method="POST" x-data="{
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    <!-- Left Column (7 cols): Main Schedule, Type, Location -->
                    <div class="lg:col-span-7 space-y-5">
                        <div class="p-5 bg-slate-50/60 dark:bg-slate-900/40 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 space-y-4">
                            <div class="flex items-center gap-2 pb-2 border-b border-slate-200/60 dark:border-slate-700/60">
                                <span class="w-2.5 h-2.5 rounded-full bg-orange-500"></span>
                                <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">1. Pengaturan Waktu & Topik</h3>
                            </div>

                            <!-- Tanggal & Jam -->
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                                <div>
                                    <label for="scheduled_date" class="block text-xs font-bold text-slate-700 dark:text-slate-300">
                                    ...
                                    <input type="date" 
                                           ...
                                           required 
                                           class="mt-1.5 block w-full rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 py-2.5 px-3 text-slate-900 dark:text-slate-100 shadow-2xs focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-xs font-semibold transition-all">
                                </div>
                                <div>
                                    ...
                                    <div class="mt-1.5 flex items-center space-x-2">
                                        ...
                                        <span class="text-xs text-slate-500 dark:text-slate-400 font-bold">WIB</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Topik Pembahasan -->
                            <div>
                                <label for="topic" class="block text-xs font-bold text-slate-700 dark:text-slate-300">
                        ...
                        <!-- Card 2: Metode & Lokasi -->
                        <div class="p-5 bg-slate-50/60 dark:bg-slate-900/40 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 space-y-4">
                            <div class="flex items-center gap-2 pb-2 border-b border-slate-200/60 dark:border-slate-700/60">
                                <span class="w-2.5 h-2.5 rounded-full bg-blue-500"></span>
                                <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">2. Metode & Tempat Bimbingan</h3>
                            </div>

                            <!-- Tipe Bimbingan Option Cards -->
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-2">
                                    Tipe Pertemuan <span class="text-orange-600">*</span>
                                </label>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <label class="relative flex cursor-pointer rounded-xl border bg-white dark:bg-slate-900 p-3 shadow-2xs focus:outline-none transition-all" :class="type === 'offline' ? 'border-orange-500 ring-2 ring-orange-500/20 bg-orange-50/30 dark:bg-orange-950/20' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300'">
                                        <input type="radio" name="type" value="offline" x-model="type" class="sr-only">
                                        <span class="flex flex-1 items-center gap-2.5">
                                            <div class="w-8 h-8 rounded-lg bg-orange-100 dark:bg-orange-900/50 text-orange-600 flex items-center justify-center shrink-0">
                                            ...
                                            <div class="flex flex-col min-w-0">
                                                ...
                                                <span class="text-[10px] text-slate-500 dark:text-slate-400 truncate">Ruang dosen / kampus</span>
                                            </div>
                                        </span>
                                        <svg class="h-4.5 w-4.5 text-orange-600 ml-1.5" :class="type === 'offline' ? 'block' : 'hidden'" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" /></svg>
                                    </label>

                                    <label class="relative flex cursor-pointer rounded-xl border bg-white dark:bg-slate-900 p-3 shadow-2xs focus:outline-none transition-all" :class="type === 'online' ? 'border-orange-500 ring-2 ring-orange-500/20 bg-orange-50/30 dark:bg-orange-950/20' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300'">
                                        <input type="radio" name="type" value="online" x-model="type" class="sr-only">
                                        <span class="flex flex-1 items-center gap-2.5">
                                            <div class="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/50 text-blue-600 flex items-center justify-center shrink-0">
                                            ...
                                            <div class="flex flex-col min-w-0">
                                                ...
                                                <span class="text-[10px] text-slate-500 dark:text-slate-400 truncate">Google Meet / Zoom</span>
                                            </div>
                                        </span>
                                        <svg class="h-4.5 w-4.5 text-orange-600 ml-1.5" :class="type === 'online' ? 'block' : 'hidden'" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" /></svg>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="flex items-center justify-between">
                                    <label for="location" class="block text-xs font-bold text-slate-700 dark:text-slate-300" x-text="type === 'online' ? 'Tautan Meeting Online' : 'Ruangan / Tempat Bimbingan'"></label>
                                ...
                                <div class="mt-1.5">
                                    <input type="text" 
                                           ...
                                           class="block w-full rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 py-2.5 px-3 text-slate-900 dark:text-slate-100 shadow-2xs focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-xs font-semibold transition-all">
                                </div>

                                <div x-show="type === 'online'" class="mt-2 space-y-2">
                                    <div x-show="meetOpened && !location" x-cloak class="p-2.5 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 rounded-xl flex items-center justify-between gap-3 text-xs text-emerald-800 dark:text-emerald-300">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span>Ruang Meet baru dibuka. Tempel tautannya di atas.</span>
                                        </div>
                                    ...
                                    <div x-show="location" x-cloak class="flex items-center justify-between p-2.5 bg-slate-100/70 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700 rounded-xl text-xs">
                                        ...
                                        <a :href="location.startsWith('http') ? location : 'https://' + location" 
                                           target="_blank" 
                                           class="inline-flex items-center gap-1 px-2.5 py-1 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-600 rounded-lg text-xs font-bold transition-all shadow-2xs">
                    ...
                    <!-- Right Column (5 cols): Mass Reschedule Toggle, Notes, Info & Actions -->
                    <div class="lg:col-span-5 space-y-5 flex flex-col justify-between">
                        <div class="space-y-4">
                            <!-- Mass Reschedule Switch Card (Prominent & Tidy) -->
                            @if(isset($relatedSessions) && $relatedSessions->count() > 0)
                                <div class="p-4.5 bg-gradient-to-br from-indigo-50/90 to-purple-50/50 dark:from-indigo-950/50 dark:to-purple-950/30 border-2 border-indigo-200/90 dark:border-indigo-800 rounded-2xl space-y-2.5 shadow-2xs">
                                    <label class="flex items-start gap-3 cursor-pointer group">
                                        <input type="checkbox" 
                                               name="apply_to_group" 
                                               value="1" 
                                               checked 
                                               class="mt-1 w-4.5 h-4.5 text-indigo-600 rounded-md border-indigo-300 dark:border-indigo-700 focus:ring-indigo-500 cursor-pointer">
                                        <div>
                                            <div class="flex items-center gap-1.5 flex-wrap">
                                                <span class="text-xs font-black text-indigo-950 dark:text-indigo-100 group-hover:text-indigo-600 transition-colors">
                                                    Terapkan ke Seluruh {{ $relatedSessions->count() + 1 }} Mahasiswa
                                                </span>
                                                <span class="px-1.5 py-0.5 bg-indigo-200 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200 rounded text-[9px] font-black uppercase">
                                                    Reschedule Massal
                                                </span>
                                            </div>
                                            <p class="text-[11px] text-indigo-800/80 dark:text-indigo-300/80 mt-1 leading-relaxed">
                                                Jadwal baru akan otomatis disinkronkan ke seluruh {{ $relatedSessions->count() + 1 }} mahasiswa dan masing-masing menerima notifikasi WhatsApp/Web.
                                            </p>
                                        </div>
                                    </label>
                                </div>
                            ...
                            <div class="p-5 bg-slate-50/60 dark:bg-slate-900/40 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 space-y-2">
                                <label for="notes" class="block text-xs font-bold text-slate-700 dark:text-slate-300">
                                    Catatan / Instruksi Persiapan <span class="text-[11px] font-normal text-slate-400">(Opsional)</span>
                                </label>
                                <textarea name="notes" 
                                          id="notes" 
                                          rows="4" 
                                          class="block w-full rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 py-2.5 px-3 text-slate-900 dark:text-slate-100 shadow-2xs focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-xs font-semibold transition-all resize-none" 
                                          placeholder="Sampaikan dokumen, bab revisi, atau bahan yang wajib dibawa mahasiswa...">{{ old('notes', $mentoringSession->notes) }}</textarea>
                            </div>

                            <!-- Reschedule Info Notice -->
                            <div class="p-3.5 bg-amber-50/80 dark:bg-amber-950/30 border border-amber-200/80 dark:border-amber-900/50 rounded-2xl flex items-start gap-2.5 text-xs text-amber-800 dark:text-amber-300">
                                ...
                                <div>
                                    <span class="font-bold text-amber-900 dark:text-amber-200">Reset Status Kehadiran:</span>
                                    <p class="mt-0.5 text-amber-700 dark:text-amber-300/90 text-[11px] leading-relaxed">
                                        Perubahan waktu akan mereset status konfirmasi mahasiswa ke <em>Menunggu Konfirmasi</em> agar mahasiswa mengonfirmasi ulang.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Action Buttons -->
                        <div class="pt-4 border-t border-slate-200/70 dark:border-slate-700 flex items-center justify-end gap-3">
