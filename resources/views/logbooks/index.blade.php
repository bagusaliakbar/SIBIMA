<x-app-layout>
    ...
    <div class="w-full space-y-6">
        ...
        @if($thesis)
            <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-7 border border-slate-200/90 dark:border-slate-700/80 shadow-xs space-y-6">
                <!-- Top Row: Tag & Status Skripsi -->
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-700/80">
                ...
                <div class="space-y-1.5">
                    ...
                    <h2 class="text-base sm:text-lg font-black text-slate-900 dark:text-white leading-relaxed">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 pt-2">
                    <!-- Pembimbing 1 -->
                    <div class="p-4 rounded-2xl bg-slate-50/80 dark:bg-slate-900/40 border border-slate-200/80 dark:border-slate-700/80 flex flex-col justify-between gap-4">
                        <div>
                            ...
                            @if($thesis->pembimbing1)
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <h4 class="text-xs font-black text-slate-900 dark:text-white truncate">{{ $thesis->pembimbing1->name }}</h4>
                        ...
                        <!-- ACC Badges P1 -->
                        <div class="pt-3 border-t border-slate-200/60 dark:border-slate-700/60 flex items-center gap-2 flex-wrap text-[10px]">
                            @if($thesis->acc_up_p1)
                        ...
                        <div>
                            ...
                            @if($thesis->pembimbing2)
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <h4 class="text-xs font-black text-slate-900 dark:text-white truncate">{{ $thesis->pembimbing2->name }}</h4>
                    ...
                    @endphp
                    <div class="p-4 rounded-2xl bg-gradient-to-br from-orange-50/70 to-amber-50/40 dark:from-slate-900/60 dark:to-orange-950/20 border border-orange-200/80 dark:border-orange-900/40 flex flex-col justify-between gap-3">
                        <div>
                        ...
                        <div class="pt-3 border-t border-orange-200/60 dark:border-orange-900/40 text-[10px] text-slate-600 dark:text-slate-300 font-medium">
                            @if($isTargetReached)
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200/90 dark:border-slate-700/80 overflow-hidden">
            <!-- Header Bar -->
            <div class="p-6 border-b border-slate-200/80 dark:border-slate-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-50/50 dark:bg-slate-900/40">
                <div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    ...
                    <a href="{{ route('logbooks.export-pdf', array_filter(['dosen' => ($filterDosen ?? 'all') !== 'all' ? $filterDosen : null])) }}" 
                       class="inline-flex items-center justify-center px-4 py-2 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider transition-all shadow-xs shrink-0" 
            ...
            @if($thesis && ($thesis->pembimbing1 || $thesis->pembimbing2))
                <div class="px-6 py-3 bg-slate-50/90 dark:bg-slate-900/60 border-b border-slate-200/80 dark:border-slate-700/80 flex items-center gap-2 overflow-x-auto">
                    <span class="text-[11px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 shrink-0 mr-1">
                        Filter Pembimbing:
                    </span>

                    <!-- Tab Semua -->
                    <a href="{{ route('logbooks.index', array_filter(['search' => $search, 'dosen' => 'all'])) }}"
                       class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 {{ ($filterDosen ?? 'all') === 'all' ? 'bg-orange-500 text-white shadow-xs' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700' }}">
                    ...
                    @if($thesis->pembimbing1)
                        <a href="{{ route('logbooks.index', array_filter(['search' => $search, 'dosen' => 'p1'])) }}"
                           class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 {{ ($filterDosen ?? 'all') === 'p1' ? 'bg-orange-500 text-white shadow-xs' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700' }}">
                    ...
                    @if($thesis->pembimbing2)
                        <a href="{{ route('logbooks.index', array_filter(['search' => $search, 'dosen' => 'p2'])) }}"
                           class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 {{ ($filterDosen ?? 'all') === 'p2' ? 'bg-orange-500 text-white shadow-xs' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700' }}">
            ...
            <!-- 4. DAFTAR RIWAYAT LOGBOOK DALAM FORMAT TIMELINE -->
            <div class="p-6">
                @if($sessions->isEmpty())
                    @if($search || ($filterDosen ?? 'all') !== 'all')
                        <div class="py-12 text-center">
                            ...
                            <a href="{{ route('logbooks.index') }}" class="inline-flex items-center mt-4 px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold hover:bg-slate-200 transition-all">
                                Reset Filter
                            </a>
                        </div>
                    @else
                        ...
                        @foreach($sessions as $session)
                            ...
                            <div class="bg-white dark:bg-slate-800/95 rounded-2xl border border-slate-200/90 dark:border-slate-700/80 p-5 sm:p-6 transition-all hover:border-orange-300 dark:hover:border-orange-500/40 hover:shadow-md hover:shadow-slate-200/30 dark:hover:shadow-none space-y-4">
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3.5 border-b border-slate-100 dark:border-slate-700/80">
                                    ...
                                    <!-- Topik Sesi -->
                                    <div>
                                        <span class="text-[9px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 block mb-0.5">Topik Diskusi / Bahasan:</span>
                                    ...
                                    <div class="p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-orange-50/50 to-amber-50/20 dark:from-slate-900/70 dark:to-orange-950/20 border border-orange-200/80 dark:border-orange-900/40 space-y-2">
                                        <div class="flex items-center justify-between gap-2">
                                            <h5 class="text-[10px] font-black text-orange-600 dark:text-orange-400 uppercase tracking-wider flex items-center gap-1.5">
                                    ...
                                    @if($session->notes)
                                        <div class="p-3 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-200/70 dark:border-slate-700/60">
                                            <span class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-0.5">Catatan Mahasiswa:</span>
                                    ...
                                    @if($session->document_path)
                                        <div class="pt-1">
                                            <a href="{{ filter_var($session->document_path, FILTER_VALIDATE_URL) ? $session->document_path : route('download.private', ['path' => $session->document_path]) }}" 
                                               target="_blank" 
                                               class="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 text-xs font-bold transition-all shadow-2xs group/doc">
                                    ...
                                    @if($session->dosen)
                                        <div class="pt-3 border-t border-slate-100 dark:border-slate-700/80 flex items-center justify-between gap-3 flex-wrap">
                                            <div class="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-[10px] text-slate-400 font-mono">{{ $session->dosen->identifier ?: 'NIP/NIDN -' }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
