        ...
        <x-table-card 
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-700">
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group">
                            ...
                            <td class="px-6 py-5 text-center">
                                ...
                                <div class="flex flex-col items-center gap-2">
                                    <div class="flex gap-1">
                                        @if($isSupervisor)
                                            <span class="px-1.5 py-0.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-[8px] font-black uppercase rounded border border-indigo-100 dark:border-indigo-500/20">Pembimbing</span>
                                        @endif
                                        @if($isExaminer)
                                            <span class="px-1.5 py-0.5 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 text-[8px] font-black uppercase rounded border border-rose-100 dark:border-rose-500/20">Penguji</span>
                                        @endif
                                    </div>
                                    @if($isExaminer)
                                        @if($revision)
                            ...
                            <td class="px-6 py-5 text-right">
                                <div class="flex justify-end gap-2">
                                    @if($exam->isGradingComplete())
                                        <a href="{{ route('defense-examiner.export-berita-acara', $exam->id) }}" 
                                           class="inline-flex items-center px-4 py-2 bg-slate-800 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all shadow-lg shadow-slate-500/20">
                                            Cetak Berita Acara
                                        </a>
                                    @endif
                    ...
                    @empty
                        <x-empty-state colspan="5" description="Anda belum diplot sebagai penguji sidang pada jadwal manapun." icon="book" />
                    @endforelse
                </tbody>
            </table>
        </x-table-card>
